...
Start();
        ...
        function vPiramida(visota, gran) {
            let S_osnovania = Math.sqrt(3)/4*Math.pow(gran,2)
            return S_osnovania/3*visota
        }
...
switch(Number(figura)){
            ...
            case 5:
                alert("ТЕБЕ СИЛЬНО/КРЕПКО ПОВЕЗЛО, ТЫ ВЫБРАЛ ПИРАМИДУ!\nСчитать будем только объем и преподаватель разрешил взять одну любую пирамиду.\n В нашем случае это будет пирамида с равнобедренным треугольником в основании.\n Далее введи высоту пирамиды и длину грани треугольника и не попутай ничего).")
                let visota1= prompt("Чему равна высота пирамиды?")
                let shirina1 = prompt("Чему равна длина грани?")
                if (Number(shirina1)&&Number(visota1)) {
                    alert(`Объем призмы равен ${vPiramida(Number(visota),Number(shirina))}.`)
                } else {
                    alert("Валенок, вводить надо числа!!!")
                    Start()
                }
